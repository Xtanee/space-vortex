using System.Linq;
using Content.Server._Vortex.Elevator;
...
public sealed class ElevatorManageFloorsCommand : IConsoleCommand
{
    ...
    private Dictionary<string, Entity<ElevatorComponent>> _elevatorCache = new();
    ...
    private Entity<ElevatorComponent>? GetElevator(string elevatorId)
    {
        UpdateElevatorCache();
        return _elevatorCache.TryGetValue(elevatorId, out var elevator) ? elevator : null;
    }

    private void HandleAdd(IConsoleShell shell, string[] args, Entity<ElevatorComponent> elevator, string elevatorId)
    {
        if (args.Length != 3)
        {
    ...
    private void HandleRemove(IConsoleShell shell, string[] args, Entity<ElevatorComponent> elevator, string elevatorId)
    {
        if (args.Length != 3)
        {
            shell.WriteLine(Loc.GetString("elevator-manage-floors-move-help", ("command", Command)));
    ...
    private void HandleList(IConsoleShell shell, string[] args, Entity<ElevatorComponent> elevator, string elevatorId)
    {
        if (args.Length != 2)
        {
            shell.WriteLine(Loc.GetString("elevator-manage-floors-move-help", ("command", Command)));
    ...
    private void HandleMove(IConsoleShell shell, string[] args, Entity<ElevatorComponent> elevator, string elevatorId)
    {
        if (args.Length != 4)
        {
            shell.WriteLine(Loc.GetString("elevator-manage-floors-move-help", ("command", Command)));
    ...
    private void UpdateElevatorCache()
    {
        ...
        var query = _entManager.AllEntityQueryEnumerator<ElevatorComponent>();
