...
public sealed class HulkGenSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<HulkGeneticsComponent, ComponentInit>(OnHulkInit);
        SubscribeLocalEvent<HulkGeneticsComponent, ComponentShutdown>(OnHulkShutdown);
        SubscribeLocalEvent<HulkGeneticsComponent, HulkChargeActionEvent>(OnHulkCharge);
    }
    ...
    #region Abilities
    private void OnHulkInit(Entity<HulkGeneticsComponent> ent, ref ComponentInit args)
    {
        foreach (var action in ent.Comp.ActionPrototypes)
            ent.Comp.ActionsEntity.Add(_action.AddAction(ent, action));
    }

    private void OnHulkShutdown(Entity<HulkGeneticsComponent> ent, ref ComponentShutdown args)
    {
        foreach (var action in ent.Comp.ActionsEntity)
            _action.RemoveAction(action);
    }

    private void OnHulkCharge(Entity<HulkGeneticsComponent> entity, ref HulkChargeActionEvent args)
    {
