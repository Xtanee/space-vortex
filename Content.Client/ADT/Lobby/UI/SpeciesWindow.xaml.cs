...
public sealed partial class SpeciesWindow : FancyWindow
{
    ...
    public void SelectSpecies(ProtoId<SpeciesPrototype> protoId)
    {
        ...
        if (Profile.JobPriorities.Count == 0)
        {
            // Default to Passenger if no job priorities are set
            job = new ProtoId<JobPrototype>("Passenger");
        }
        else
        {
            var highJobs = Profile.JobPriorities.Where(x => x.Value == JobPriority.High);
            if (highJobs.Any())
            {
                job = highJobs.First().Key;
            }
            else
            {
                job = Profile.JobPriorities.First().Key;
            }
        }

        var previewProfile = Profile.WithSpecies(protoId);
